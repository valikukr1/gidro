...
class WFSpellCheckerPlugin extends WFEditorPlugin 
{
	...
	*/
	...
	function getConfig()
	{
		...
		if (!is_array($config)) {
			...
			$config = array(
				...
				'general.remote_rpc_url'=> 'http://speller.yandex.net/services/tinyspell',
				// PSpell settings
	...
	private function remoteRPC()
	{
		...
		$raw = JRequest::getString('json');
		// Convert request to original TinyMCE format
		$raw = str_replace('"fn":',   '"id":"c0","method":', $raw);
		$raw = str_replace('"args":', '"params":', $raw);
